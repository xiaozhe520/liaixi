...
class Ad
{
	 ...
	 * @return string
	 */
	public static function show($position,$city='',$goods_cat_id = 0)
	{
		...
		if($positionObject)
		{
			$adList = self::getAdList($positionObject['id'],$city,$goods_cat_id);
...
OEF;
	 ...
	 * @return array
	 */
	public static function getAdList($position,$city,$goods_cat_id = 0)
	{
		$now    = date("Y-m-d H:i:s",ITime::getNow());
		$adDB   = new IModel("ad_manage");
		return $adDB->query("city='$city' and position_id={$position} and goods_cat_id = {$goods_cat_id} and start_time < '{$now}' AND end_time > '{$now}' ORDER BY `order` ASC ");
	}
