...
class _userInfo extends pluginBase
{
	...
	public function userRegAct()
	{
		...
		if($reg_option == 1)
		{
			...
			$title = "老公发";
			//$title = "账号激活邮件";
			$url = "http://www.iwebshop.com/index.php?controller=simple&action=modify&id=$user_id";
			$content = "亲爱的：下班了记得快点回家记得吃饭哟！爱你哟！么么哒！";
			//$content = "用户您好，这是一封账户激活邮件，请点击下面的链接激活您的账户，<a href='".$url."'>".$url."</a>此邮箱为系统自动发送，请勿回复，如果您未注册本站账户，请忽略此邮件。";
			//调用send方法发送邮箱
			if ($mail->send($email,$title,$content)) {
			  echo "<script>alert('已向您的邮箱发送一封账户激活邮件，请前往邮箱激活！');</script>";
			}else{
				echo $mail->getError();
			}
			return;
