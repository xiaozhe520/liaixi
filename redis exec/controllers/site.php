...
class Site extends IController
{
    public $layout='site';
    //新增
    private $client_id='101353491';//ID
    private $redirect_uri='http://www.iwebshop.com/index.php';//回调地址
    private $scope='uuzxhfi';//随机数
    private $appkey='df4e46ba7da52f787c6e3336d30526e4';//key
	function init()
	{
    ...
    public function long(){
        $url="https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=$this->client_id&redirect_uri=$this->redirect_uri&scope=$this->scope";
        header("location:$url");
    }
	...
	function index()
	{
		if (isset($_GET['code'])) {
			$code = $_GET['code'];
			// echo $code;
			$url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=$this->client_id&client_secret=$this->appkey&code=$code&redirect_uri=$this->redirect_uri";
			$str=file_get_contents($url);
			//echo $str;
			$fest=strpos($str,'=');
			$last=strpos($str, '&');
			$token=substr($str,$fest+1,$last-$fest-1);
			// echo $token;
			...
			$fest=strpos($str,'(');
			$last=strpos($str, ')');
			$data=substr($str,$fest+1,$last-$fest-1);
			$data=json_decode($data,true);
			$openid=$data['openid'];
			// echo $openid;
			$url="https://graph.qq.com/user/get_user_info?access_token=$token&oauth_consumer_key=$this->client_id&openid=$openid";
			$str = file_get_contents($url);
	    ...
	    $ip= '114.80.116.240';
		//$ip = '139.199.126.253';
		$ak = 'dTlT0ESMCuXosSfGWgfbx1NPe4xUT00m';
