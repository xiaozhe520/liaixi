...
class Ucenter extends IController implements userAuthorization
{
    ...
    public function addAddress(){
        $name=IReq::get('name');
        $map = new IModel('map');
        $map->setData(['name'=>$name]);
        $map->add();
    }
    //取出数据得到经纬度
    public function shou(){
       $name=IReq::get('name');
       $url="http://api.map.baidu.com/geocoder/v2/?address=$name&output=json&ak=dTlT0ESMCuXosSfGWgfbx1NPe4xUT00m&callback=showLocation"; 
       $str=file_get_contents($url);
       // echo "$str";
       $left=strpos($str,'(');     // 获取“（”在左侧第一次出现的位置
        $right=stripos($str,')');     // 获取“)”在右侧第一次出现的位置
        $str=substr($str,$left+1,$right-$left-1);      // 截取左右括号之间的字符 
        echo $str;
    }
}