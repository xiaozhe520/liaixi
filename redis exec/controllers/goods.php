...
class Goods extends IController implements adminAuthorization
{
	...
	public function receiveFile(){
		//1接受数据
		$id = IReq::get('id');//编号
		$name = IReq::get('name');//原名称
		$cnt = IReq::get('cnt');//总切片数
		// print_r($id);die;
		//2.移动文件
		$file=$_FILES['blob'];//获取切片信息
		// print_r($file);die;
		$file_name=$file['tmp_name'];//切片临时文件
		$fang = 'upload/video/'.$name.'_'.$id;
		move_uploaded_file($file_name, $fang);
        // 3.合并文件
        if ($id==$cnt) {
        	$str = '';
         for ($i=1; $i <=$cnt; $i++) { 
         	$file_name = 'upload/video/'.$name.'_'.$i;
         	$str.=file_get_contents($file_name);
         }
         file_put_contents("upload/video/".$name,$str);
	...
	//商品查找
	public function goods_list_10(){
		//第一种
		//$goods = new IModel('goods');
		//$data = $goods->query("name like '%服装%'",'id,name,sell_price,img','sell_price desc',10);
	    //第二种
	    $goods = new IQuery('goods');
	    $goods->fields='*';
	    $goods->where="name like '%服装%'";
	    $goods->order='sell_price desc';
	    $goods->limit=10;
	    $data=$goods->find();
	    $this->setRenderData(['data'=>$data]);
	    $this->redirect('goods_list_10');
	}
}
